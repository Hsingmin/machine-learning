...
# -*- author: Hsingmin Lee
#
# ocr_model.py -- Construct ocr model with Keras framework.

import codecs
from keras.utils import plot_model
from keras.models import Model
from keras.layers import Input, Conv2D, MaxPooling2D, ZeroPadding2D
from keras.layers import BatchNormalization, Dense, TimeDistributed, Flatten
from keras.layers import Permute, Bidirectional, GRU
...
def get_model(height,nclass):
    ...
    # Zeros padding for image width, and no padding for height. 
    # (None, 8, None, 256)
    ...
    # Get the whole sequence of Bidirectional GRU by concat Bi-GRU outputs.
    # (None, None, 512)
...
def decode(pred):
        ...
        for i in range(length):
            if t[i] != n and (not (i > 0 and t[i - 1] == t[i])):
                        char_list.append(charactersS[int(t[i])])
...
def show_model():
    height = 32
    nclass = 1024
    ctc_model, crnn_model = get_model(height, nclass)
    print(ctc_model.summary())
    print(crnn_model.summary())
    plot_model(ctc_model, to_file='ctc_model_graph.png')
    plot_model(crnn_model, to_file='crnn_model_graph.png')
