...
def get_weights_regularizer(weights, regularizer):
	if regularizer != None:
		tf.add_to_collection('losses', regularizer(weights))

def interface(input_tensor, avg_class, regularizer):
	...
	with tf.variable_scope('layer1', reuse=tf.AUTO_REUSE):
		...
		get_weights_regularizer(weights, regularizer)
...
def train(mnist):
	...
	y = interface(x, avg_class=None, regularizer=regularizer)
	...
	average_y = interface(x, variable_averages, regularizer)
